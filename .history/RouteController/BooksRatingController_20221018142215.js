    ...
    try {
        booksRating_db.remove({ _id: req.params.id }, (err, deletedData) => {
            ...
            if (err) {
                ...
                resp.send({ message: "Book Deleted Successfully" })
            }
            else {
...
route.get('/search/:name', async (req, resp) => {
     try {
        let word= new RegExp([req.body.name],req.body.name);
        console.log("working",word)
        await booksRating_db.find({book: word} , (err, searchedData) => {
            if (err) {
                return resp.status(500).json({ message: "Error " + err })
            }
